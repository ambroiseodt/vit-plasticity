...
@dataclass
class DomainNetldDatasetConfig:
    r"""
...
class DomainNetldDataset(Dataset):
    r"""
    ...
    def __init__(self, config: DomainNetldDatasetConfig):
        super().__init__()
        ...
        if train:
            # Recover training indices
            train_pool_indices = indices[:n_train]
            train_samples = samples[train_pool_indices]

            # Recover labels
            train_labels = train_samples[:, 1].astype(int)

            # 3. Deterministic low-data subsampling
            sample_size = 3450
            ...
            for c in range(self.n_classes):
                class_mask = np.where(train_labels == c)[0]
                if len(class_mask) > 0:
                    class_mask = np.random.permutation(class_mask)
                    selected = train_pool_indices[class_mask[:samples_per_class]]
                    low_data_indices.extend(selected)
            np.random.set_state(st0)
            indices = np.array(low_data_indices)[:sample_size]
        else:
