r"""
Codebase to analyze the plasticity of MAE components.
...
class MaeConfig:
    ...
    def __post_init__(self):
        # Valid model size
        if self.model_size not in ["base", "large", "huge"]:
...
class Mae(nn.Module):
    def __init__(self, config: MaeConfig):
        ...
        self.model_name = f"facebook/vit-mae-{config.model_size}"
    ...
    def _load_from_huggingface(self, model_name: str, device: str) -> PreTrainedModel:
        ...
        model = ViTMAEModel.from_pretrained(f"{model_name}", device_map=device)
        model.eval()
        return model
    ...
    def get_decomposition(self, x: torch.Tensor) -> dict:
        ...
        # Embedding layer
        out, _, _ = self.model.embeddings(x)
        ...
        for i, block in enumerate(layers):
            # LN1
            block_out = block.layernorm_before(out)
            key = "attn_norm"
            outputs[f"block{i}" + "_" + key] = move_to_cpu(block_out)

            # MHA
            block_out = block.attention(out)
            key = "attn"
            outputs[f"block{i}" + "_" + key] = move_to_cpu(block_out)

            # LN2
            block_out = block.layernorm_after(out)
            key = "ffn_norm"
            outputs[f"block{i}" + "_" + key] = move_to_cpu(block_out)

            # FC2
            block_out = block.intermediate.dense(out)
            key = "ffn_fc1"
            outputs[f"block{i}" + "_" + key] = move_to_cpu(block_out)
            ...
            block_out = block.output.dense(expanded_out)
            key = "ffn_fc2"
            outputs[f"block{i}" + "_" + key] = move_to_cpu(block_out)
...
def analysis(config: AnalysisConfig) -> None:
    ...
    model_config = MaeConfig(model_size=config.model_size, device=config.device)
    model = Mae(config=model_config)
    ...
    # Save config
    config_name = f"analysis_mae_{config.model_size}_{config.dataset_name}"
