...
def apply_lora(model: nn.Module, rank: int = 8) -> nn.Module:
    r"""
    Apply LoRA on query and value projections of all unfrozen attention layers.

    Only layers with at least one trainable parameter are targeted, meaning
    LoRA is automatically restricted to components not frozen by freeze_model.
    Standard convention: lora_alpha = 2 * rank.

    Parameters
    ----------
    model:
        Vision transformer model, after freeze_model has been called.
    rank: int, default=8
        LoRA rank. Typical values: 4, 8, 16.
    ...
    """
    # Target only unfrozen linear layers matching attention q/v projections
    target_modules = []
...
def eval(config: EvaluationConfig) -> None:
    with ExitStack() as context_stack:
        ...
        if "lora_rank" in exp_config:
            model = apply_lora(model=model, rank=exp_config["lora_rank"])
            total = sum(p.numel() for p in model.parameters())
            trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            logger.info(f"LoRA (rank={8}): {trainable:,} trainable / {total:,} total ({100 * trainable / total:.2f}%)")
